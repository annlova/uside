...
#ifndef USIDE_BNF_INTERPRETER_H
#define USIDE_BNF_INTERPRETER_H
...
namespace parser {
    ...
    public:
        int interpret(bnf::Grammar* grammar);

    private:
        struct Env : public bnf::Env {
            int a;
            int b;
        };
        ...
        struct ListDefVisitor : public bnf::ListDefVisitor {
            void visit(const bnf::ListDefEpsilon& token, bnf::Env& env) const override;

            void visit(const bnf::ListDefList& token, bnf::Env& env) const override;
        };
