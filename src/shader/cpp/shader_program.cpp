...
GLuint shader::ShaderProgram::msBoundHandle = mscBoundHandleUnboundValue;
...
void shader::ShaderProgram::bind() const {
    ASSERT_MSG(mHandle != msBoundHandle, "Shader program already bound.");
...
void shader::ShaderProgram::unbind() const {
    ASSERT_MSG(mHandle == msBoundHandle, "Shader program already unbound.");
    msBoundHandle = mscBoundHandleUnboundValue;

    glUseProgram(mscBoundHandleUnboundValue);
    ASSERT_GL();
}
